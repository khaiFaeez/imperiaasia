...
export default {
    ...
    data(){
        return {
            isPending: this.client.invoices.find(o => o.Status_Inv === 'PENDING')
        }
    }
}
...
<template>
    ...
    <AppLayout>
        ...
        <div class="flex flex-row items-center justify-end mb-5 gap-3">
            <Link :href="route('portfolio.client.edit',{client:client.id})"
            ...
            <table class="table table-compact table-bordered w-full">
                ...
                <tbody>
                    <tr v-for="invoice in $page.props.client.invoices" :key="invoice.Id">
                        ...
                        <td>
                            <Link class="hover:underline text-primary hover:cursor-pointer"
                                :href="route('portfolio.invoice.show',{invoice:invoice.Id})"> {{ invoice.Inv_No }}
                            </Link>
                        </td>
                        <td>{{ invoice.Date ? moment(moment().subtract(invoice.Aging, 'days')).endOf('day').fromNow() :
                            ""
                            }}</td>
                        <td>{{ invoice.Date ? moment(invoice.Date).format('DD/MM/YYYY') : "" }}</td>
                        <td v-if="!isPending && hasAnyPermission(['invoice-create'])">
                            <Link
                                :href="route('portfolio.invoice.repeat',{portfolio:route().params.portfolio,invoice_id:invoice.Id})"
