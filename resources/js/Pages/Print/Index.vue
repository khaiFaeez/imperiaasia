...
<template>
    ...
    <AppLayout>
        ...
        <div class="flex items-center justify-end gap-4">
            <form :action="route('portfolio.print.invoices')" method="post" target="_blank" @submit.prevent="check_form"
            ...
            <form :action="route('portfolio.print.dockets')" method="post" target="_blank" @submit.prevent="check_form"
                v-if="$page.props.invoices.data.length > 0">
                <input type="hidden" :value="$page.props.attribute_name" name="_token" />
                <input type="hidden" :value="selected" name="ids" />
