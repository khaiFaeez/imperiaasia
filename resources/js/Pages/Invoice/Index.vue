...
export default {
    ...
    methods: {
        goToViewPage(data, event) {
            this.$inertia.get(
                route('portfolio.invoice.show', { invoice: data.Id })
            )
    ...
    <AppLayout>
        ...
        <div class="overflow-auto shadow-xl">
            <table class="table table-compact table-bordered w-full  ">
                ...
                <tbody>
                    <tr class="hover hover:cursor-pointer" v-for="invoice in $page.props.invoices.data"
                        :key="invoice.Id" @click="goToViewPage(invoice, event)">
                        <td></td>
                        <td>
                        ...
                        <td>
                            {{
                                    invoice.Promise_pay1
                                        ? moment(invoice.Promise_pay1).format('LL')
                                        : ''
                            }}
                        </td>
                        <td>
                            {{
                                    invoice.Promise_pay2
                                        ? moment(invoice.Promise_pay2).format('LL')
                                        : ''
                            }}
                        </td>
                        <td>
                            {{
                                    invoice.Promise_pay3
                                        ? moment(invoice.Promise_pay3).format('LL')
                                        : ''
                            }}
                        </td>
                        <td>
                            {{
                                    invoice.Promise_pay4
                                        ? moment(invoice.Promise_pay4).format('LL')
                                        : ''
                            }}
                        </td>
                        ...
                        <td>
                            {{
                                    invoice.Created_Date
                                        ? moment(invoice.Created_Date).format('LL')
                                        : ''
                            }}
                        </td>
                    </tr>
